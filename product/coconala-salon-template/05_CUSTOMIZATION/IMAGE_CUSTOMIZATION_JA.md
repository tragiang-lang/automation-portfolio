...
本ガイドは画像**そのものの差し替え**方法です。ギャラリーの
**見た目のレイアウト**（グリッド/レンガ状/主役写真つきなど）を
変更したい場合は
[`DESIGN_CUSTOMIZATION_JA.md`](DESIGN_CUSTOMIZATION_JA.md#4-個別レイアウト設定)（「4.5
ギャラリーレイアウト」）
を参照してください（画像を差し替えるだけであれば、レイアウトの変更は
不要です）。
