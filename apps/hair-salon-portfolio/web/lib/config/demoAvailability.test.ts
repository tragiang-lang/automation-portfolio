...
// monday closed, tuesday 10:00-19:00, friday 10:00-20:00.
const MONDAY_CLOSED = "2026-09-14";
...
const LONG_SERVICE_ID = "SV004"; // 90 minutes (カラー)
const SHORT_SERVICE_ID = "SV005"; // 30 minutes (トリートメント)
...
describe("getDemoAvailability", () => {
  it("is a pure synchronous function — no network/Calendar access", () => {
    const result = getDemoAvailability({ serviceId: SHORT_SERVICE_ID, date: TUESDAY });
    expect(result).not.toBeInstanceOf(Promise);
  });

  it("returns identical slots for identical input (determinism)", () => {
    const first = getDemoAvailability({ serviceId: SHORT_SERVICE_ID, staffId: "ST001", date: TUESDAY });
    const second = getDemoAvailability({ serviceId: SHORT_SERVICE_ID, staffId: "ST001", date: TUESDAY });
    expect(second).toEqual(first);
  });

  it("returns no slots on a day the demo business is closed", () => {
    expect(getDemoAvailability({ serviceId: SHORT_SERVICE_ID, date: MONDAY_CLOSED })).toEqual([]);
  });
  ...
  it("still returns slots on a normal open weekday (existing behavior unchanged)", () => {
    const slots = getDemoAvailability({ serviceId: SHORT_SERVICE_ID, date: TUESDAY });
  ...
  it("every slot fits within business hours once the service duration is added", () => {
    const slots = getDemoAvailability({ serviceId: LONG_SERVICE_ID, date: TUESDAY });
    expect(slots.length).toBeGreaterThan(0);
  ...
  it("a shorter service can use later start times than a longer one on the same day", () => {
    const longSlots = getDemoAvailability({ serviceId: LONG_SERVICE_ID, date: FRIDAY });
    const shortSlots = getDemoAvailability({ serviceId: SHORT_SERVICE_ID, date: FRIDAY });
  ...
  it("every returned slot satisfies the AvailableTimeSlot shape", () => {
    const slots = getDemoAvailability({ serviceId: SHORT_SERVICE_ID, date: FRIDAY });
    for (const slot of slots) {
  ...
  it("stays deterministic across different staff selections", () => {
    const withStaffA = getDemoAvailability({ serviceId: SHORT_SERVICE_ID, staffId: "ST001", date: FRIDAY });
    const withStaffARepeat = getDemoAvailability({ serviceId: SHORT_SERVICE_ID, staffId: "ST001", date: FRIDAY });
    expect(withStaffARepeat).toEqual(withStaffA);

    const withStaffB = getDemoAvailability({ serviceId: SHORT_SERVICE_ID, staffId: "ST002", date: FRIDAY });
    const withStaffBRepeat = getDemoAvailability({ serviceId: SHORT_SERVICE_ID, staffId: "ST002", date: FRIDAY });
